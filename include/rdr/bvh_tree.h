...
use_median_heuristic:
    ...
    std::nth_element(nodes.begin() + span_left, nodes.begin() + split,
        nodes.begin() + span_right, [dim](const auto &a, const auto &b) {
          const AABB &a_aabb = a.getAABB();
          const AABB &b_aabb = b.getAABB();
          return (a_aabb.low_bnd[dim] + a_aabb.upper_bnd[dim]) <
                 (b_aabb.low_bnd[dim] + b_aabb.upper_bnd[dim]);
        });
